    ...
    for (let z = Math.max(0, zoom - 1); z <= Math.min(18, zoom + 1); z++) {
      ...
      for (let dx = -radius; dx <= radius; dx++) {
        for (let dy = -radius; dy <= radius; dy++) {
          const tileX = x + dx
          const tileY = y + dy
          const url = `https://{s}.tile.openstreetmap.org/${z}/${tileX}/${tileY}.png`
          tiles.push(url)
        }
      }
    }
...
// Component to handle route visualization with offline support
const RouteVisualization = ({ 
  ...
  }, [currentLocation, destination, isOnline, onRouteUpdate])
...
// Component for turn-by-turn navigation
const TurnByTurnNavigation = ({ 
  ...
  useEffect(() => {
    ...
    if (currentStepIndex >= 0) {
      setCurrentStep(routeData.steps[currentStepIndex])
